 ...
 * @LastEditTime: 2022-10-06 19:40:57
 */
...
// 添加当前路由对应component
function createComponent(absPath, name) {
  return `component: () => import(/* webpackPrefetch: true, webpackChunkName: '${name}' */ "${absPath}"), \n`;
}
...
// 添加当前路由对应布局组件
function createLayout(absPath, name) {
  return `layout: () => import(/* webpackPrefetch: true, webpackChunkName: '${name}' */ "${absPath}"), \n`;
}
  ...
  if (fs.statSync(dir).isDirectory()) {
    ...
    while (content.length) {
      ...
      // 如果当前路径是layout.j|tsx, 则当作路由的布局组件
        ...
        !hasName && (result += `name: '${name}', \n`);
        !hasPath && (result += createPath(routerPath, level, replace));
        result += createLayout(absPath, name);
        hasName = true;
        hasPath = true;
